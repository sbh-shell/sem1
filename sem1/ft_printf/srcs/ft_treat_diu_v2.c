...
/*   Updated: 2014/11/30 00:15:24 by sbenning         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_build_d(t_llint arg, t_expr *expr, size_t nb_d)
{
	...
	if (arg < 0 || (arg > 0 && (expr->flag[ESCAPE] || expr->flag[PLUS])))
	{
...
size_t			ft_treat_d(va_list *va, t_expr *expr)
{
	...
	if (arg < 0 || (expr->flag[ESCAPE] || expr->flag[PLUS]))
		nb_d++;
	len = (expr->width > nb_d) ? expr->width : nb_d;
	len = (expr->precision > len) ? expr->precision : len;
	len += ((expr->flag[ESCAPE] || expr->flag[PLUS]) && (expr->precision > expr->width)) ? 1 : 0;
